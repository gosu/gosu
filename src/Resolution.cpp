...
namespace
{
    SIZE calculate_available_size()
    {
        ...
        SIZE available_size;
        available_size.cx = 2 * (work_area.right - work_area.left) - (window_size.right - window_size.left);
        available_size.cy = 2 * (work_area.bottom - work_area.top) - (window_size.bottom - window_size.top);
        return available_size;
    }

    SIZE available_size = calculate_available_size();
}
...
namespace
{
    SDL_DisplayMode current_display_mode = { 0, 0 };
}
