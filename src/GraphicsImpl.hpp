...
namespace Gosu
{
    ...
    template<typename T>
    bool is_p_to_the_left_of_ab(T xa, T ya,
        T xb, T yb, T xp, T yp)
    {
    ...
    void reorder_coordinates_if_necessary(T& x1, T& y1,
    ...
    bool is_entity(const std::wstring& name);
    const Bitmap& entity_bitmap(const std::wstring& name);
