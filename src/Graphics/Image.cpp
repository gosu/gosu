...
Gosu::Image::Image(const std::wstring& filename, bool tileable)
{
	// Forward.
	Bitmap bmp;
	loadImageFile(bmp, filename);
	Image(bmp, tileable).data.swap(data);
}
    ...
    bool tileable)
{
	...
	Bitmap bmp;
	loadImageFile(bmp, filename);
	Image(bmp, srcX, srcY, srcWidth, srcHeight, tileable).data.swap(data);
}

Gosu::Image::Image(const Bitmap& source, bool tileable)
{
	// Forward.
	Image(source, 0, 0, source.width(), source.height(), tileable).data.swap(data);
}
    ...
    int tileWidth, int tileHeight, bool tileable)
{
    ...
    for (int y = 0; y < tilesY; ++y)
        for (int x = 0; x < tilesX; ++x)
            images.push_back(Image(bmp, x * tileWidth, y * tileHeight, tileWidth, tileHeight, tileable));
    ...
    int tileWidth, int tileHeight, bool tileable)
{
    Bitmap bmp;
    loadImageFile(bmp, filename);
    return loadTiles(bmp, tileWidth, tileHeight, tileable);
}
