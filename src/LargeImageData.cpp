                                     ...
                                     unsigned image_flags)
{
    ...
    tiles_x = (m_w + tile_width - 1) / tile_width;
    tiles_y = (m_h + tile_height - 1) / tile_height;
                                ...
                                ZPos z, BlendMode mode) const
{
    ...
    for (int ty = 0; ty < tiles_y; ++ty) {
        ...
        for (int tx = 0; tx < tiles_x; ++tx) {
            ImageData& tile = *m_tiles[ty * tiles_x + tx];

            double rel_x_l = x / m_w;
            double rel_x_r = (x + tile.width()) / m_w;
            double rel_y_t = y / m_h;
            double rel_y_b = (y + tile.height()) / m_h;
...
#define LERP2D(what, x_weight, y_weight)                                                           \
    lerp(lerp(what##1, what##3, y_weight), lerp(what##2, what##4, y_weight), x_weight);
            ...
            tile.draw(x_t_l, y_t_l, c_t_l, x_t_r, y_t_r, c_t_r, x_b_l, y_b_l, c_b_l, x_b_r, y_b_r,
                      c_b_r, z, mode);

            x += tile.width();
        }
        y += m_tiles[ty * tiles_x]->height();
    }
}
                                                                ...
                                                                int height) const
{
    ...
    int y = 0;
    for (int ty = 0; ty < tiles_y; ++ty) {
        int row_height = m_tiles[ty * tiles_x]->height();
        ...
        for (int tx = 0; tx < tiles_x; ++tx) {
            ...
            int sub_right = std::min(tile.width(), left + width - x);
            int sub_bottom = std::min(tile.height(), top + height - y);

            sub_tiles.emplace_back(
                    tile.subimage(sub_left, sub_top, sub_right - sub_left, sub_bottom - sub_top));
